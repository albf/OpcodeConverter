...
extern uint32_t PC;
extern uint32_t nPC;
extern uint32_t mark[VM_MEMORY_SZ];
...
/*void vm_cpu_2_indirect_pure_opt3(uint32_t newPC,int opt)
{
	...
	static void *array[] = {  &&calcop,  &&branch,//40  
	...
	static void *array2 [] = {  &&sll,   &&error, &&srl,   &&sra,   &&sllv,
...
/*
void vm_cpu_2_indirect_inpure_opt3(uint32_t newPC,int opt)
{
	PC = newPC;
	nPC = 4;
	RF[0] = 0; //Register $zero must always be zero
