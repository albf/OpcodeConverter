...
// 5.1 + 5.2 A
uint32_t VM_memory2[VM_MEMORY_SZDIVR4];
void vm_cpu_2_mem2(uint32_t newPC,int opt, int i) 
{
	...
	for(k=3; k<i; k+=4) {
	  VM_memory2[k/4] = (((uint32_t)VM_memory[k-3] <<24) | ((uint32_t)VM_memory[k-2] <<16) | ((uint32_t)VM_memory[k-1] <<8) | ((uint32_t)VM_memory[k])); 
	}
...
#define FIX_OP 43
// 5.1 + 5.2 B + 5.4
extern int8_t mark3[VM_MEMORY_SZ];
void vm_cpu_2_mem2_all_opt4(uint32_t newPC,int opt)
{
	...
	while (!halted) 
	{
		if(mark3[PC] < 0) {
		  ...
		  *((uint32_t *) (&(VM_memory[PC]))) = instr;
		  mark3[PC] = op;
		}
		else {
		  instr = *((uint32_t *)(&(VM_memory[PC])));
		  op = mark3[PC];
		}
		...
		switch (op) 
		{
					...
					case 11: { // movn	001010 				if(rt!=0) rd = rs
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						if (RF[rt] != 0) RF[rd] = RF[rs];
					...
					case 10: { // movz	001010 				if(rt==0) rd = rs
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						if (RF[rt] == 0) RF[rd] = RF[rs];
					...
					case 32: { // add		100000	ArithLog	$d = $s + $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] + RF[rt];
					...
					case 33: { // addu	100001	ArithLog	$d = $s + $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] + RF[rt];
					...
					case 36: { // and		100100	ArithLog	$d = $s & $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] & RF[rt];
					...
					case 26: { // div		011010  DivMult		lo = $s / $t; hi = $s % $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						LO = RF[rs] / RF[rt];
						HI = RF[rs] % RF[rt];
					...
					case 27: { // divu	011011  DivMult		lo = $s / $t; hi = $s % $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						LO = RF[rs] / RF[rt];
						HI = RF[rs] % RF[rt];  
					...
					case 24: { // mult	011000  DivMult		hi:lo = $s * $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						mult = (uint64_t)RF[rs] * (uint64_t) RF[rt];
						HI = (mult >> 32) & 0xFFFFFFFF;
					...
					case 25: { // multu	011001	DivMult		hi:lo = $s * $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						mult = (uint64_t)RF[rs] * (uint64_t) RF[rt];
						HI = (mult >> 32) & 0xFFFFFFFF;
					...
					case 39: { // nor		100111	ArithLog	$d = ~($s | $t)
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = ~(RF[rs] | RF[rt]);
					...
					case 37: { // or		100101	ArithLog	$d = $s | $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] | RF[rt];
					...
					case 4: { // sllv	000100	ShiftV		$d = $t << $s
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rt] << RF[rs];
					...
					case 7: { // srav	000111	ShiftV		$d = $t >> $s
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rt] >> RF[rs];
					...
					case 6: { // srlv	000110	ShiftV		$d = $t >>> $s
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rt] >> RF[rs];  
					...
					case 34: { // sub		100010	ArithLog	$d = $s - $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] - RF[rt];  
					...
					case 35: { // subu	100011	ArithLog	$d = $s - $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] - RF[rt];    
					...
					case 38: { // xor		100110  ArithLog	$d = $s ^ $t
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = RF[rs] ^ RF[rt];    
					...
					case 42: { // slt		101010	ArithLog	$d = ($s < $t)
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = (RF[rs] < RF[rt])?1:0;
					...
					case 43: { // sltu	101011	ArithLog	$d = ($s < $t)
						rs = (instr >> 21) & 0x1F;
						rt = (instr >> 16) & 0x1F;
						rd = (instr >> 11) & 0x1F;
						RF[rd] = (RF[rs] < RF[rt])?1:0;
					...
					case 9: { // jalr	001001	JumpR		$31 = pc; pc = $s
						rs = (instr >> 21) & 0x1F;
						RF[31] = PC+8;
						PC = nPC;
						nPC = RF[rs];
					...
					case 16: { // mfhi	010000	MoveFrom	$d = hi
						rd = (instr >> 11) & 0x1F;
						RF[rd] = HI;
						PC  =  nPC;
						nPC  += 4;
					...
					case 18: { // mflo	010010	MoveFrom	$d = lo
						rd = (instr >> 11) & 0x1F;
						RF[rd] = LO;
						PC  =  nPC;
						nPC  += 4;
					...
					case 17: { // mthi	010001	MoveTo		hi = $s
						rs = (instr >> 21) & 0x1F;
						HI = RF[rs];
						PC  =  nPC;
						nPC  += 4;
					...
					case 19: { // mtlo	010011	MoveTo		lo = $s
						rs = (instr >> 21) & 0x1F;
						LO = RF[rs];
						PC  =  nPC;
						nPC  += 4;
					...
					case 12: { // syscall 	001100	syscall		$2		seen on the .s generated by the ecc compiler
						if (syscall((uint8_t)RF[2])) //registers $4 and $5 useb by hallcall (sensid and retval respectively)
						{
							halted = 1; //Syscall returned 1, exit code
						}
			...
			//Immediate encoding
			case 51: { //addi    001000  ArithLogI       $t = $s + SE(i)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = RF[rs] + immediate; //Implementar trap!
			...
			case 52: { //addiu   001001  ArithLogI       $t = $s + SE(i)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = (RF[rs] + immediate);
			...
			case 55: { //andi    001100  ArithLogI       $t = $s & ZE(i)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = RF[rs] & (uint32_t)immediate;
			...
			case 56: { //ori     001101  ArithLogI       $t = $s | ZE(i)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = RF[rs] | (uint32_t)immediate;
			...
			case 57: { //xori    001110  ArithLogI       $d = $s ^ ZE(i)
				rs = (instr >> 21) & 0x1F;
				rd = (instr >> 11) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rd] = RF[rs] ^ (uint32_t)immediate;
			...
			case 58: 	 //lui	   001111          Rdest, imm: Load Upper Immediate
			case 68: { //lhi     011001  LoadI   HH ($t) = i 
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = ((uint32_t)immediate) << 16;
				PC  =  nPC;
			...
			case 67: { //llo     011000  LoadI   LH ($t) = i
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = immediate;
				PC  =  nPC;
			...
			case 53: { //slti    001010  ArithLogI       $t = ($s < SE(i))
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = (RF[rs] < immediate)?1:0;
			...
			case 54: { //sltiu   001011  ArithLogI       $t = ($s < SE(i)) 
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = (RF[rs] < immediate)?1:0;
			...
			case 47: { //beq     000100  Branch  if ($s == $t) pc += i << 2
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				if (RF[rs] == RF[rt])
				{
			...
			case 50: { //bgtz    000111  BranchZ if ($s > 0) pc += i << 2
				rs = (instr >> 21) & 0x1F;
				if ((int32_t)RF[rs] > 0)
				{
					immediate = (instr >> 0) & 0xFFFF;
			...
			case 49: { //blez    000110  BranchZ if ($s <= 0) pc += i << 2
				rs = (instr >> 21) & 0x1F;
				if ((int32_t)RF[rs] <= 0)
				{
					immediate = (instr >> 0) & 0xFFFF;
			...
			case 48: { //bne     000101  Branch  if ($s != $t) pc += i << 2
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				if (RF[rs] != RF[rt])
				{
			...
			case 75: { //lb      100000  LoadStore       $t = SE (MEM [$s + i]:1)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = ((uint32_t)VM_memory[RF[rs] + immediate]& 0x7F)  | (uint32_t)(VM_memory[RF[rs] + immediate] & 0x80)<<24; //Load byte carrying signal to the register
			...
			case 79: { //lbu     100100  LoadStore       $t = ZE (MEM [$s + i]:1)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = ((uint32_t)VM_memory[RF[rs] + immediate]) & 0xFF;
			...
			case 76: { //lh      100001  LoadStore       $t = SE (MEM [$s + i]:2)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = ((((uint32_t)(VM_memory[RF[rs] + immediate])<< 16) | ((uint32_t)VM_memory[RF[rs] + immediate + 1])) & 0x7FFF) | (uint32_t)((VM_memory[RF[rs] + immediate + 1]) & 0x8000) << 16;
			...
			case 80: { //lhu     100101  LoadStore       $t = ZE (MEM [$s + i]:2)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = ((((uint32_t)(VM_memory[RF[rs] + immediate])<<16) | ((uint32_t)VM_memory[RF[rs] + immediate + 1])) & 0xFFFF);
			...
			case 78: { //lw      100011  LoadStore       $t = MEM [$s + i]:4
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	RF[rt] = ((uint32_t)(VM_memory[RF[rs] + immediate]) << 24) | ((uint32_t)(VM_memory[RF[rs] + immediate + 1]) << 16) | ((uint32_t)(VM_memory[RF[rs] + immediate + 2]) << 8) | ((uint32_t)VM_memory[RF[rs] + immediate + 3]);
			...
			case 83: { //sb      101000  LoadStore       MEM [$s + i]:1 = LB ($t)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	VM_memory[RF[rs] + immediate] = (uint8_t)(RF[rt] & 0xFF);
			...
			case 84: { //sh      101001  LoadStore       MEM [$s + i]:2 = LH ($t)
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	VM_memory[RF[rs] + immediate] = (uint8_t)((RF[rt] & 0xFF00) >> 8);
			...
			case 86: { //sw      101011  LoadStore       MEM [$s + i]:4 = $t
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	VM_memory[RF[rs] + immediate] = (uint8_t)((RF[rt] & 0xFF000000) >> 24);
			...
			case 46: { //jal     000011  Jump    $31 = pc; pc = i << 2
#if DEBUGING
				printf(">>RA:%x\tAddress:%x\n", RF[31], address<<2);
#endif
				RF[31] = PC + 8;
			...
			case 69: { //trap    011010  Trap    Dependent on operating system; different values for immed26 specify different operations. See the list of traps for information on what the different trap codes do.
				address = (instr >> 0) & 0x3FFFFFF;
				syscall((uint8_t)(address&0xFF));
				PC  =  nPC;
				nPC  += 4;
			...
			case 85: { //swl Rsrc1, imm(Rsrc2): Store Word Left
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				VM_memory[RF[rs] + immediate] = (uint8_t)((RF[rt] & 0xFF000000) >> 24);
			...
			case 89: { //swr Rsrc1, imm(Rsrc2): Store Word Right
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
			  	VM_memory[RF[rs] + immediate] = (uint8_t)(RF[rt] & 0xFF);
			...
			case 77: { //lwl Rdest, imm(Rsrc): Load Word Left
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = ((uint32_t)(VM_memory[RF[rs] + immediate]) << 24) | ((uint32_t)(VM_memory[RF[rs] + immediate + 1]) << 16);
			...
			case 81: { //lwr Rdest, imm(Rsrc): Load Word Right
				rs = (instr >> 21) & 0x1F;
				rt = (instr >> 16) & 0x1F;
				immediate = (instr >> 0) & 0xFFFF;
				RF[rt] = ((uint32_t)(VM_memory[RF[rs] + immediate -1]) << 8) | ((uint32_t)VM_memory[RF[rs] + immediate]);
...
void vm_cpu_2_mem2_all(uint32_t newPC,int opt)
{
	...
	while (!halted) 
	{
		if(mark3[PC] == 0) {
		  mark3[PC] = 1;
		  *((uint32_t *) (&(VM_memory[PC]))) =  (((uint32_t)VM_memory[PC] <<24) | ((uint32_t)VM_memory[PC+1] <<16) | ((uint32_t)VM_memory[PC+2] <<8) | ((uint32_t)VM_memory[PC+3]));
		}
		instr = *((uint32_t *)(&(VM_memory[PC])));
		//instr = VM_memory2[PC];
...
/*
{
	...
	{
		//instr = fetch(PC);
		//instr2 = (((uint32_t)VM_memory[PC] <<24) | ((uint32_t)VM_memory[PC+1] <<16) | ((uint32_t)VM_memory[PC+2] <<8) | ((uint32_t)VM_memory[PC+3]));
		...
		switch (op) 
		{
			...
			case 0b100011: { //lw      100011  LoadStore       $t = MEM [$s + i]:4
				...
				immediate = (instr >> 0) & 0xFFFF;
			  	//RF[rt] = ((uint32_t)(VM_memory[RF[rs] + immediate]) << 24) | ((uint32_t)(VM_memory[RF[rs] + immediate + 1]) << 16) | ((uint32_t)(VM_memory[RF[rs] + immediate + 2]) << 8) | ((uint32_t)VM_memory[RF[rs] + immediate + 3]);
				RF[rt] = VM_memory2[(RF[rs] + immediate)/4];
				PC  =  nPC;
				nPC  += 4;
				break;
			}
